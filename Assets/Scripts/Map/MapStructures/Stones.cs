...
public class Stones : MapStructure
{
    Texture2D text;
    ...
    public override void PlantOnMap(MapGen map, Vector2 size, int seed)
    {
        ...
        for (int y = 0; y < size.y; y++)
        {
            ...
            {
                float sample =
                    PerlinNoise.GetNoiseValue(x, y, 50, seed) * 0.6f +
                    PerlinNoise.GetNoiseValue(x, y, 100, seed * 2) * 0.3f +
                    PerlinNoise.GetNoiseValue(x, y, 75, seed * 4) * 0.1f;
    ...
    IEnumerator PlantTrees(Vector2 size, MapGen map)
    {
        ...
        text.SetPixels(pix);
        //File.WriteAllBytes("kamienie.png", text.EncodeToPNG());
        text.Apply();
